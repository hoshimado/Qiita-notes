...
以降、セッションから「`req.session.passport.user`」を取得できれば「ログイン状態」と言えるので、OIDC認証を掛けたい「`/auth`」配下へのアクセス時は、例えば「`req.session.passport.user.profile`が存在するならば、`src/auth/`配下の静的ファイルを表示する」などのように制御すればよい（本サンプルでは、そのような制御としている）。
